...
const BFS_STEP = 8;          // each BFS level shifts a trait by this much
const BFS_THRESHOLD = 22;    // max Euclidean distance to "match" an avatar
const MAX_BFS_LEVELS = 6;    // safety cap
...
/**
 * BFS in trait-space.
 * Returns { matched: avatarKey, levels: number, visited: number, path: string[] }
 */
function bfsMatch(userScores, avatars) {
  ...
  while (queue.length > 0) {
    ...
    // Check if this node is within threshold of any avatar
    let bestAvatar = null;
    ...
    for (const nb of bfsNeighbours(node)) {
      ...
      if (!visited.has(key)) {
        visited.add(key);
        // Label which trait shifted for path display
...
function TraitBar({ value, color, label, pct, delay = 0 }) {
  ...
  useEffect(() => { const t = setTimeout(() => setW(value), 120 + delay); return () => clearTimeout(t); }, [value]);
  return (
...
/* ════════════════ BFS VISUALISER ════════════════
   Shows the BFS exploration result as a mini
   "trait graph" — each ring = one BFS level.
   Avatars appear at their discovered level.
*/
function BFSVisualiser({ result, userScores }) {
  ...
  // Build per-avatar distances for display
...
export default function MirrorAI() {
  ...
  function finishQuickTest() {
    ...
    // Convert avg to rough trait scores for BFS
    const userScores = {
  ...
  function selectAvatar(key) {
    ...
    const bfs = bfsMatch(av.traits, AVATARS); // BFS from avatar's own traits
  ...
  return (
    <div style={{ display: "flex", height: "100vh", overflow: "hidden", background: S.bg }}>
      ...
      <aside style={{
        ...
        <div style={{ marginTop: "auto", padding: "16px 18px", borderTop: `1px solid ${S.border}`, display: "flex", alignItems: "center", gap: 10 }}>
          <div style={{ width: 34, height: 34, borderRadius: "50%", background: `linear-gradient(135deg,${S.accent},${S.accent2})`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18, flexShrink: 0 }}>
            {profile ? profile.emoji : "🪞"}
          </div>
          <div>
            <div style={{ fontSize: 13, fontWeight: 600 }}>Alex Chen</div>
            <div style={{ fontSize: 11, color: S.muted }}>{profile ? `${profile.mbti} · ${profile.name}` : "Pick an avatar ✦"}</div>
          </div>
        </div>
      </aside>
